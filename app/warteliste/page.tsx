...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const loadLabels = async () => {
      ...
      const waitlistLabel = loadedLabels.find(
        (label: Label) => label.name?.toLowerCase() === "warteliste"
      );
      if (waitlistLabel) {
        setLabelFilters((prev) => (prev.length ? prev : [waitlistLabel.id]));
      }
    };
    loadLabels();
  }, []);
  ...
  const filteredContacts = useMemo(() => {
    ...
    if (onlyDue) {
      ...
      filtered = filtered.filter(
        (contact) =>
          contact.follow_up_at && new Date(contact.follow_up_at) <= now
      );
    }
    return filtered;
  }, [contacts, labelFilters, onlyDue]);
  ...
  return (
    <AppShell
      title="Warteliste"
      subtitle="Kontakte auf Warteliste"
      action={<LogoutButton />}
    >
      ...
      <div className="sticky top-0 z-30 min-w-0 space-y-3 bg-base-950/95 pb-3 pt-3 backdrop-blur md:static md:bg-transparent md:pb-0 md:pt-0">
        ...
        <section className="grid gap-4 rounded-lg border border-base-800 bg-base-850 p-4 md:grid-cols-6">
          ...
          <label className="flex items-center gap-2 text-sm text-text-muted">
            <input
              type="checkbox"
              className="h-4 w-4 rounded border border-base-800 bg-base-900 text-emerald-200"
              checked={onlyDue}
              onChange={(event) => setOnlyDue(event.target.checked)}
            />
            Nur fällige Follow-ups
          </label>
